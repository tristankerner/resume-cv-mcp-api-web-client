...
over HTTP. Additional clients — a TUI, a native mobile app, whatever comes
next — get their own repositories rather than living here; see
`resume-cv-mcp-api`'s README for what exists.

None of the API contract is duplicated here. This README says which endpoints
this client uses in passing, but the actual contract — request/response
shapes, auth, error handling — lives in `resume-cv-mcp-api`'s own docs.
...
opening this client — see `resume-cv-mcp-api`'s `.env.example` and the
"Browser client" section of its `README.md`. Every failure mode below traces
back to this one setting:
...
All three are the gate — CI runs them on every push and pull request (see
`.github/workflows/test.yml`). `npm run test` covers the pure logic only —
...
that repo's README for the full chain. `resume-cv-mcp-api`'s own Docker
Compose file for running with this client baked in follows the same
